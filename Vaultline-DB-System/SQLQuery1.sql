...
UPDATE WALLETS SET BALANCE = 10000.00  WHERE WalletId = 5003;

EXEC sp_PerformTransfer
@SenderWalletId = 5001,
@ReceiverWalletId = 5003,
@Amount = 3000.00;

UPDATE WALLETS SET Balance = 200000.00 WHERE WalletId = 5003;

EXEC sp_PerformTransfer
    @SenderWalletId = 5003,
    @ReceiverWalletId = 5004,
    @Amount = 120000.00;
SELECT * FROM WALLETS;
--------------------------------------------------------------------------------------------------3rd function
...
SELECT 
    ...
    t.TransactionDate
FROM TRANSACTIONS t
INNER JOIN WALLETS wSender ON t.SenderWalletId = wSender.WalletId
INNER JOIN USERS uSender ON wSender.UserID = uSender.UserId
INNER JOIN WALLETS wReceiver ON t.ReceiverWalletId = wReceiver.WalletId
INNER JOIN USERS uReceiver ON wReceiver.UserID = uReceiver.UserId;
GO
